...
def create_controller(model, data, torque=0.067):
    # model/data не используем, но оставляем единый интерфейс
    return ConstantTorqueController(constant_torque=torque)
...
def analyze_and_plot(logs):
    ...
    plt.axhline(
        y=controller.target_speed,
        color="b",
        linestyle="--",
        linewidth=2,
        label=f"Целевая: {controller.target_speed:.3f} м/с",
    )
    plt.fill_between(
        time_steps,
    ...
    plt.axvline(
        ...
        linestyle="-",
        linewidth=2,
        label=f"Средняя: {avg_speed:.3f} м/с",
    )
    ...
    plt.fill_between(
        ...
        color="red",
        alpha=0.3,
        label=f"Ниже целевого (<{low_speed_threshold:.3f})",
    )
    ...
    plt.fill_between(
        ...
        color="green",
        alpha=0.3,
        label=f"Выше целевого (>{high_speed_threshold:.3f})",
    )
    ...
    plt.suptitle(
        f"СИМУЛЯЦИЯ С ПОСТОЯННЫМ МОМЕНТОМ ({controller.constant_torque} Н·м)",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
    ...
    plt.axhline(
        ...
        linestyle="--",
        linewidth=2.5,
        label=f"Целевая скорость: {controller.target_speed:.3f} м/с",
    )
    ...
    plt.title(
        f"Анализ скорости ровера (постоянный момент: {controller.constant_torque} Н·м)",
        fontsize=14,
        fontweight="bold",
    )
    plt.grid(True, alpha=0.3)
    ...
    stats_text = (
        f"Приложенный момент: {controller.constant_torque} Н·м\n"
